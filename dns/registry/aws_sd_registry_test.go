...
func TestAWSSDRegistryTest_Records(t *testing.T) {
	p := newInMemoryProvider([]*endpoint.Endpoint{
		...
		newEndpointWithOwnerAndDescription("foo2.test-zone.example.org", "1.2.3.4", endpoint.RecordTypeA, "owner", "\"heritage=external-ips,external-ips/owner=owner\""),
		newEndpointWithOwnerAndDescription("foo3.test-zone.example.org", "my-domain.com", endpoint.RecordTypeCNAME, "", ""),
		newEndpointWithOwnerAndDescription("foo4.test-zone.example.org", "my-domain.com", endpoint.RecordTypeCNAME, "owner", "\"heritage=external-ips,external-ips/owner=owner\""),
	}, nil)
